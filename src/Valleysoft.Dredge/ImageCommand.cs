...
public class ImageCommand : Command
{
    ...
    public class OsCommand : Command
    {
        ...
        internal static async Task<(WindowsOsInfo Info, string Repo)?> GetWindowsOsInfoAsync(Image imageConfig, string baseLayerDigest,
            IDockerRegistryClientFactory dockerRegistryClientFactory)
        {
            ...
            using IDockerRegistryClient mcrClient =
                await dockerRegistryClientFactory.GetClientAsync(McrRegistry);
    ...
    public class DockerfileCommand : Command
    {
        ...
        public async Task<string> GetMarkupStringAsync(string image, bool noColor, bool noFormat)
        {
            ...
            if (isWindows)
            {
                ...
                dockerfileBuilder.AppendLine($"FROM {McrRegistry}/{repo}:{windowsOsInfo.Version}-{imageConfig.Architecture}");
            }
            else
            {
        ...
        private async Task<IEnumerable<LayerHistory>> GetWindowsLayersAsync(Image imageConfig, DockerManifestV2 manifest, string windowsRepo)
        {
            using IDockerRegistryClient mcrClient =
                await dockerRegistryClientFactory.GetClientAsync(McrRegistry);
        ...
        private async Task<(WindowsOsInfo Info, string Repo)> GetWindowsInfoAsync(DockerManifestV2 manifest, Image imageConfig)
        {
