...
public static class DockerfileGenerator
{
    ...
    {
        ImageName parsedImageName = Core.ImageName.Parse(imageName);
        using IDockerRegistryClient client = await dockerRegistryClientFactory.GetClientAsync(parsedImageName.Registry);
        DockerManifestV2 manifest = (await ManifestHelper.GetResolvedManifestAsync(client, parsedImageName, appSettings, options.PlatformOptions)).Manifest;
        ...
        ImageConfig imageConfig = await client.Blobs.GetImageAsync(parsedImageName.Repo, digest);
        ...
        if (isWindows)
        {
            (WindowsOsInfo windowsOsInfo, string repo) = await GetWindowsInfoAsync(manifest, imageConfig, dockerRegistryClientFactory);
            layers = await GetWindowsLayersAsync(imageConfig, manifest, repo, dockerRegistryClientFactory);
        ...
        foreach (LayerHistory layerHistory in layers)
        {
            ...
            string line = GetHistoryLine(layerHistory.CreatedBy, ref currentShell, options);
    ...
    private static string GetHistoryLine(string line, ref string? currentShell, DockerfileOptions options)
    {
    ...
    private async static Task<(WindowsOsInfo Info, string Repo)> GetWindowsInfoAsync(
        DockerManifestV2 manifest, ImageConfig imageConfig, IDockerRegistryClientFactory dockerRegistryClientFactory)
    {
        ...
        var windowsOsInfo = await OsHelper.GetWindowsOsInfoAsync(
    ...
    private async static Task<IEnumerable<LayerHistory>> GetWindowsLayersAsync(ImageConfig imageConfig, DockerManifestV2 manifest, string windowsRepo,
